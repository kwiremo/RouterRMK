 ...
 * 2. The LL3PDaemon needs to have a reference to this class.  The Manager should have a getter
 *    method to return the Messenger object to the LL3PDaemon. The LL3PDaemon calls this class's
...
public class Messenger implements Runnable {
    ...
    private ArrayList<NameAndNetwork> allNamesAndNetworks;// an array list with hardcoded student and moticon numbers
    ...
    public void finishCreatingMessenger(){
        ...
        //TODO: I changed this to getParentActivity.
        parentActivity = ParentActivity.getParentActivity();
        names = new ArrayList<String>();	// a fixed array of strings with names.
        spinnerNames = new ArrayList<NameAndNetwork>();	// an array of objects which match a com.renemoise.routerrmk.network to a student name
        receivedMessagelist = new ArrayList<ChatMessage>();  // a list of received messages. Each message contains the name and message.
        addNames(); // build the names array, and then populate the com.renemoise.routerrmk.network/name array as well.
        ...
        layer3Daemon = LL3PDaemon.getInstance();
    ...
    public void openMessengerWindow(){
        ...
        //   show students actively in the com.renemoise.routerrmk.network.
    ...
    private View.OnClickListener sendMessage = new View.OnClickListener() {
        ...
        public void onClick(View v) {
			...
			/*
			 * use the spinner position as an index into the spinner adapter to get the com.renemoise.routerrmk.network from that
			 * spinner item (each spinner item has both a name and com.renemoise.routerrmk.network number).
			 */
            Integer networkNumber = spinnerAdapter.getItem(spinnerPosition).getNetwork();
            // pull the com.renemoise.routerrmk.network number out of the ll3p address of the name selected by the spinner.
    ...
    /**
     * Receive a message. It is expected that the com.renemoise.routerrmk.UI Manager will call this method when a message is
     *    to be posted on the screen.   The LL3P Daemon will pass the message to the com.renemoise.routerrmk.UI Manager, whicih
    ...
    public void receiveMessage(Integer ll3pAddress, String payload){
        if (messengerWindow.isShowing()){
            int network = (Integer) (ll3pAddress/256); // com.renemoise.routerrmk.network portion = address/256 with no remainder.

            // note that in original "256" comes from com.renemoise.routerrmk.network constants class.
    ...
    /**
     * the name and com.renemoise.routerrmk.network class is a private class used to create an object
     * where the name/com.renemoise.routerrmk.network pairs can be stored and used as a single object
     * by the spinner.
     * @author pat
     *
     */
    private class NameAndNetwork{
        ...
        private Integer network;	// com.renemoise.routerrmk.network for this user.
    ...
    private void resetSpinner(){
        validNetworkList = FIB.getTableArrayList(); // get active nodes from forwarding information table.
        ...
        //TODO: I changed the compareTO to  "==" because I uses primitive data type "int" rather than using "Integer".
        for (TableRecord entry : validNetworkList){
            if (((RoutingRecord)entry).getNetworkNumber() > -1 &&
                ((RoutingRecord) entry).getNetworkNumber() < 16)
                spinnerAdapter.add(allNamesAndNetworks.get(((RoutingRecord) entry).getNetworkNumber()-1)); // com.renemoise.routerrmk.network numbers go from 1-15, array elements from 0-14
        }
    ...
    private class MessageAdapter extends ArrayAdapter<ChatMessage> {
        ...
        /**
         * the get view class is called when the com.renemoise.routerrmk.UI (the android com.renemoise.routerrmk.UI, not UIManager)
